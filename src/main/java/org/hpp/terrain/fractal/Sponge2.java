...
// 3D fractal Sponges with translations set,  Evgeny Demidov  10 Oct 2001
import java.awt.*;
import java.awt.event.*;
import java.util.StringTokenizer;

public class Sponge2 extends java.applet.Applet
        implements MouseListener, MouseMotionListener {

    int nVert, nFace, nTran, it, h, w, h2, w2, mx0, my0, xPol[], yPol[], Zsort[];
    double fiX = .2, fiY = .3, dfi = .01, scale, scl;
    int[][] face;
    double[][] vert, vert1, Norm, tran, tran1;
    double[] Norm1z;
    Image buffImage;
    ...
    public void init() {
        ...
        nTran = Integer.parseInt(st.nextToken());
...
//System.out.println("Hi");
        tran = new double[nTran][3];
        tran1 = new double[nTran][3];
        s = getParameter("Tran");
        st = new StringTokenizer(s, " ,");
        for (int i = 0; i < nTran; i++) {
            tran[i][0] = Double.valueOf(st.nextToken()).doubleValue();
            tran[i][1] = Double.valueOf(st.nextToken()).doubleValue();
            tran[i][2] = Double.valueOf(st.nextToken()).doubleValue();
        }
        face = new int[nFace][];
        int tmp[] = new int[30];
        Zsort = new int[nTran];
    ...
    public void rotate() {
        ...
        for (int i = 0; i < nTran; i++) {
            tran1[i][0] = m00 * tran[i][0] + m02 * tran[i][2];
            tran1[i][1] = m10 * tran[i][0] + m11 * tran[i][1] + m12 * tran[i][2];
            tran1[i][2] = m20 * tran[i][0] + m21 * tran[i][1] + m22 * tran[i][2];
        }
        for (int i = 0; i < nFace; i++) {
            Norm1z[i] = m20 * Norm[i][0] + m21 * Norm[i][1] + m22 * Norm[i][2];
        }

        for (int i = nTran - 1; --i >= 0;) {
            boolean flipped = false;
            for (int j = 0; j <= i; j++) {
                int a = Zsort[j], b = Zsort[j + 1];
                if (tran1[a][2] > tran1[b][2]) {
                    Zsort[j + 1] = a;
                    Zsort[j] = b;
                    flipped = true;
                }
    ...
    public void recursion(int n, double Xo, double Yo, double sc, double tr) {
        if (n == 0) {
            drawPolygon(Xo, Yo, sc);
        } else {
            for (int i = 0; i < nTran; i++) {
                int is = Zsort[i];
                recursion(n - 1, Xo + tran1[is][0] * tr, Yo + tran1[is][1] * tr, sc / scl, tr / scl);
            }
        }
    }
