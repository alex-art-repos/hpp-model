...
// 3D Gauss mountains,  Evgeny Demidov  10 Oct 2001
import java.awt.*;
import java.awt.event.*;
import java.util.StringTokenizer;

public class Lorenz3D extends java.applet.Applet
        implements MouseListener, MouseMotionListener {

    int n = 2, n1, h, w, h2, w2, mx0, my0, xPol[], yPol[];
    double fiX = .2, fiY = .3, dfi = .01, scale = 1, m20, m21, m22;
    ...
    public void setup() {
        ...
        double n2 = n / 2., dx = w2 / n2;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                double x = dx * i - w2, z = dx * j - w2;
                vert[i][j][0] = x;
                vert[i][j][1] = Fun(x, z);
                vert[i][j][2] = z;
            }
    ...
    public double Fun(double x, double z) {
        return 2000000. / (x * x + z * z + 10000.) - 100.;
    }
